...
exports.saveUsuario = function(usuario) {                                                                                                                                                       
    return database.one('insert into usuario(idcliente, email, senha, data_criacao) values($1, $2, $3, $4) returning *', [ usuario.idcliente, usuario.email, encrypt(Buffer.from(usuario.senha, 'utf8')), usuario.data_criacao]);
}
 ...
 */
 exports.getPassword = function(email) {
    const pass = database.query('select senha from usuario where email = $1 ', [email]); 
    return pass;
}